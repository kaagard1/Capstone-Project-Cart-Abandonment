...
Our number of carts stayed the same, as did our total number of observations. 
...
All of these items have an abandonment rate of higher than our baseline.
...
The end result was a single row per `CART_ID`,  our target variable `ABANDON`, and 148 columns for all of the unique event names.
...
The DBSCAN model identified three non-noise clusters with an average silhouette score of redacted, indicating that observations within each cluster are, on average, more similar to one another than to those in other clusters.
